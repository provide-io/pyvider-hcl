...
def _get_baseline_updates() -> dict[str, int]:
    """Get the baseline updates dict from the pytest-discovered conftest module.

    Pytest may load this conftest under a different module name than
    'tests.memray.conftest', so we need to find the canonical instance.
    """
    # Find all loaded modules that are this conftest
    for mod in sys.modules.values():
        if (
            hasattr(mod, "_baseline_updates")
            and hasattr(mod, "_get_baseline_updates")
            and mod is not sys.modules.get(__name__)
            and getattr(mod, "__file__", None) == __file__
        ):
            return mod._baseline_updates
    return _baseline_updates
...
def _save_memray_baseline_updates(request: Any) -> None:
    ...
    yield
    updates = _get_baseline_updates()
    updates.update(_baseline_updates)
    if os.getenv("MEMRAY_UPDATE_BASELINE") and updates:
        baseline_path = Path(__file__).parent / "baselines.json"
