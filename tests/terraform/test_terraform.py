...
class TestHeredocsThroughTheBlockParser:
    ...
    `config.py` reaches hcl2 through `to_dict()` rather than `hcl2.loads`, so it
    has to pass `HCL2_OPTIONS` itself. Nothing else notices if it stops: the
    values simply arrive as raw `<<EOT ... EOT` source, and the two parse paths
    disagree in silence.
    """
    ...
    def test_a_heredoc_in_a_block_body_is_resolved(self) -> None:
        ...
        assert config.block_at("locals").body["body"] == "line\n"
    ...
    def test_both_paths_agree(self) -> None:
        ...
        assert config.block_at("locals").body["body"] == loaded["locals"][0]["body"]
