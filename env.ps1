# env.ps1 - pyvider-hcl Development Environment Setup
#
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the pyvider-hcl root directory"
    exit 1
}
...
$Profile = if ($env:PYVIDER_HCL_PROFILE) { $env:PYVIDER_HCL_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/pyvider-hcl_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "pyvider-hcl_setup"
...
Write-Host "Installing pyvider-hcl in editable mode..." -NoNewline
try {
...
Get-ChildItem -Path $ParentDir -Directory -Filter "wrkenv" | ForEach-Object {
    $SiblingName = $_.Name
    Write-Host "Installing $SiblingName..." -NoNewline
    try {
        & uv pip install --no-deps -e $_.FullName 2>&1 | Out-File -FilePath (Join-Path $LogDir "$SiblingName.log")
...
Write-Host "`n$("pyvider-hcl development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  pyvider-hcl --help  # pyvider-hcl CLI"
